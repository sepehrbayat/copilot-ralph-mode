...
""", encoding='utf-8')
    ...
    # Create groups
    (groups_dir / "enterprise.json").write_text(json.dumps({
        "id": "enterprise",
...
class TestChaosEngineering:
    ...
    def test_rapid_state_changes(self, ralph: RalphMode, isolated_workspace: Path):
        ...
        for cycle in range(20):
            ralph.enable(
                f"Rapid cycle {cycle}",
                max_iterations=5,
                completion_promise=f"DONE_{cycle}"
            )
    ...
    def test_unicode_edge_cases_in_chaos(self, ralph: RalphMode, isolated_workspace: Path):
        ...
        for i, problematic in enumerate(problematic_strings):
            try:
                ralph.enable(
                    f"Task with {problematic}",
                    max_iterations=5,
                    completion_promise=f"DONE_{i}"
                )
...
class TestStressScenarios:
    ...
    def test_large_batch_processing(self, ralph: RalphMode):
        ...
        tasks = [
            {"id": f"task-{i:04d}", "title": f"Task {i}", "prompt": f"Process item {i}"}
            for i in range(100)
        ]
...
class TestRealWorldScenarios:
    ...
    def test_batch_code_review_workflow(self, ralph: RalphMode, isolated_workspace: Path):
        ...
        review_tasks = [
            ...
            {
                "id": "review-db",
                "title": "Review database.py",
                "prompt": "Review database queries for SQL injection"
            },
            {
                "id": "review-config",
...
class TestPerformanceBaseline:
    ...
    def test_batch_init_performance_baseline(self, ralph: RalphMode):
        ...
        for count in task_counts:
            tasks = [
                {"id": f"task-{i}", "title": f"Task {i}", "prompt": f"Do {i}"}
