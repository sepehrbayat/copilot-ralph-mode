...
""", encoding='utf-8')
    ...
    # Create task groups
    (groups_dir / "backend.json").write_text(json.dumps({
        "id": "backend",
    ...
    (groups_dir / "quality.json").write_text(json.dumps({
        "id": "quality",
...
class TestFullWorkflowE2E:
    ...
    def test_batch_workflow_complete(self, production_workspace: Path):
        ...
        """
        rm = RalphMode(base_path=production_workspace)
        library = TaskLibrary(production_workspace / "tasks")
    ...
    def test_interrupted_workflow_recovery(self, production_workspace: Path):
        ...
        rm1.enable(
            "Long running task with important progress",
            max_iterations=50,
            completion_promise="LONG_TASK_DONE"
        )
...
class TestTaskLibraryE2E:
    ...
    def test_full_task_library_workflow(self, production_workspace: Path):
        ...
        assert rm.is_active()
        assert task['title'] in rm.get_prompt() or task['content'] in rm.get_prompt()
